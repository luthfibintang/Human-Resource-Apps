...
async function displayReimbursementData() {
    ...
    try {
        ...
        const { data: userData, error: userError } = await supabase
            .from('users')
            .select('id_user, name')
            .in('id_user', data.map(item => item.id_user));
        ...
        data.forEach((reimbursement, index) => {
            const user = userData.find(user => user.id_user === reimbursement.id_user);
                    ...
                    <td class="text-center">
                        <div class="action-label">
                            ...
                            <div class="dropdown-menu dropdown-menu-right">
                                <a class="dropdown-item change-status" data-id="${reimbursement.id_reimbursement}" data-status="Approved" href="#"><i class="fa fa-dot-circle-o text-success"></i> Approved</a>
                                <a class="dropdown-item change-status" data-id="${reimbursement.id_reimbursement}" data-status="Declined" href="#"><i class="fa fa-dot-circle-o text-danger"></i> Declined</a>
                            </div>
                        </div>
                    </td>
                    <td>${approvedBy}</td>
        ...
        document.querySelectorAll('.change-status').forEach(item => {
            item.addEventListener('click', async (event) => {
                ...
                const reimbursementID = event.target.dataset.id;
                const newStatus = event.target.dataset.status;
                await updateUserStatus(reimbursementID, newStatus);
            });
        });
...
async function updateUserStatus(overtimeId, newStatus) {
    const currentUser = await getCurrentUser();
    if (!currentUser) {
        alert('User not logged in');
        return;
    ...
    try {
        const { data, error } = await supabase
            ...
            .eq('id_reimbursement', overtimeId);
...
async function updateReimbursementStatistics() {
    try {
        ...
        // Get Reimbursement amount for this month
        ...
        if (monthlyReimbursementErr) {
            throw new Error('Error fetching Reimbursement amount:', monthlyReimbursementErr.message);
        }
