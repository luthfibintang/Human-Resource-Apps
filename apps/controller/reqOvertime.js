...
async function displayOvertimeData() {
    ...
    try {
        ...
        data.forEach((overtime, index) => {
            const user = userData.find(user => user.id_user === overtime.id_user);
                    ...
                    <td class="text-center">
                        <div class="action-label">
                            ...
                            <div class="dropdown-menu dropdown-menu-right">
                                <a class="dropdown-item change-status" data-id="${overtime.id_overtime}" data-status="Approved" href="#"><i class="fa fa-dot-circle-o text-success"></i> Approved</a>
                                <a class="dropdown-item change-status" data-id="${overtime.id_overtime}" data-status="Declined" href="#"><i class="fa fa-dot-circle-o text-danger"></i> Declined</a>
                            </div>
                        </div>
                    </td>
                    <td>${approvedBy}</td>
        ...
        document.querySelectorAll('.change-status').forEach(item => {
            item.addEventListener('click', async (event) => {
                ...
                const overtimeId = event.target.dataset.id;
                const newStatus = event.target.dataset.status;
                await updateUserStatus(overtimeId, newStatus);
            });
        });
...
// Overtime Statistics
async function updateOvertimeStatistics() {
    try {
